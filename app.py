...
if section == "📷 Image Doubt":
    ...
    if uploaded_image is not None:
        ...
        # Optional: Apply adaptive threshold to enhance OCR
        processed = cv2.adaptiveThreshold(open_cv_image, 255,
                                          cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                          cv2.THRESH_BINARY, 11, 2)

        # Save temp and re-read using pytesseract
        cv2.imwrite("temp_img.png", processed)
        extracted_text = pytesseract.image_to_string(Image.open("temp_img.png"))

        # Show extracted text
        st.markdown("#### ✨ Extracted Question from Image:")
        if not extracted_text.strip():
            st.warning("⚠ Couldn't extract readable text. Try uploading a clearer image.")
            st.stop()

        # Allow user to edit OCR text
        edited_question = st.text_area("✍️ Review or Edit the Extracted Question:", value=extracted_text.strip())

        # Ask Tutor Button
        if st.button("🤖 Ask STEM Tutor from Image"):
            with st.spinner("Thinking..."):
                prompt = f"""
You are a highly knowledgeable and friendly STEM tutor. A student has submitted the following question as an image. 

Your task:
1. Identify the subject (Physics, Chemistry, Math, etc.)
2. Break down the question.
3. Provide a clear and step-by-step explanation or solution.
...
"""
                ...
                try:
                    ...
                    if "choices" in json_data and json_data["choices"]:
                        ...
                        st.error("⚠ No answer returned. Please try again or check your input.")
